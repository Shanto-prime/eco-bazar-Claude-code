...
test.beforeAll(async () => {
  prisma = newPrisma();
  const u = await prisma.user.findFirst({ where: { username: USER.username }, select: { id: true } });
...
async function seedUserCart() {
  const product = await prisma.product.findFirst({ select: { slug: true, name: true, price: true } });
  const items = [{ slug: product.slug, name: product.name, price: product.price / 100, qty: 1 }];
...
test.describe("Fix 1 — saved address prefills checkout", () => {
  ...
  test("the address book offers Bangladeshi divisions, not US states", async ({ page }) => {
    ...
    const divisionOptions = await page.locator('select[name="state"] option')
      .evaluateAll((os) => os.map((o) => o.textContent.trim()));

    // The old US option set must be gone...
    expect(divisionOptions.join(" ")).not.toMatch(/Illinois|California|New York/);
    // ...and replaced by the Bangladesh hierarchy, labelled "English (বাংলা)".
    expect(divisionOptions.some((o) => /Dhaka|Barisal|Chattogram/.test(o))).toBe(true);
  ...
  test("a saved Division/District/Thana prefills all three checkout selectors", async ({ page }) => {
    await page.goto("/dashboard/settings", { waitUntil: "networkidle" });
    ...
    await page.getByRole("button", { name: /add address|save address/i }).last().click();

    // Confirm it persisted with all three levels, thana included.
    await expect
      .poll(() => prisma.address.findFirst({
        where:  { userId, street: STREET },
        select: { state: true, city: true, thana: true, country: true },
      }), { timeout: 15_000 })
      .toEqual({ state: chosen.state, city: chosen.city, thana: chosen.thana, country: "Bangladesh" });

    // Now the payoff: checkout must come up already filled in.
    await seedUserCart();
    await page.goto("/checkout", { waitUntil: "networkidle" });
    await expect(page.locator('select[name="division"]')).toHaveValue(chosen.state);
    await expect(page.locator('select[name="district"]')).toHaveValue(chosen.city);
    await expect(page.locator('select[name="thana"]')).toHaveValue(chosen.thana);
    await expect(page.locator('input[name="street"]')).toHaveValue(STREET);
  });

  test("geography saved before the fix does not poison the prefill", async ({ page }) => {
    // Exactly the corrupt shape the old code produced.
    await prisma.address.create({
      data: {
        userId, firstName: "Legacy", lastName: "Row", street: STREET,
        state: "Illinois", city: "DHAKA", country: "USA", isDefault: true,
      },
    });
    await seedUserCart();
    await page.goto("/checkout", { waitUntil: "networkidle" });
