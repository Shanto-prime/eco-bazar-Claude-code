        ...
        return (
            <div>
                <div className="text-sm mb-4">
                    <Link href="/dashboard/products" className="text-eco-green">
                        {t("dashboard.backToProducts")}
                    </Link>
                </div>
                <div className="rounded-md bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3">
                    {t("dashboard.cantEditProduct")}
                    <b>{product.createdBy?.name || product.createdBy?.email}</b>
    ...
    return (
        <div>
            ...
            <div className="text-sm text-gray-500 mb-6">
                ...
                <b>{product.createdBy?.name || product.createdBy?.email}</b>
                {t("dashboard.addedOn")}
                <LocalTime value={product.createdAt} />
            </div>
            <ProductForm
                product={{
                    ...product,
                    price: toDollars(product.price),
                    oldPrice: toDollars(product.oldPrice),
                }}
                action={boundUpdate}
                allowDelete
