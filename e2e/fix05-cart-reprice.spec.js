...
test.beforeAll(async () => {
    ...
    if (stalePrice.toFixed(2) === realPrice)
        stalePrice = Number(realPrice) + 3.01;
});
...
async function cartPriceFor(page, stalePrice, staleName = "STALE NAME") {
    ...
    await seedGuestCart(page, [
        { slug: SLUG, name: staleName, price: stalePrice, qty: 1 },
    ]);
    ...
    await expect
        .poll(
            async () =>
                (await page.locator("body").innerText()).includes(realPrice),
            { timeout: 15_000 },
        )
        .toBe(true);
...
test.describe("Fix 5   the cart reprices from the database", () => {
    test("a stale LOWER price is corrected upward (the expired-offer case)", async ({
        page,
    }) => {
    ...
    test("a cart item whose product no longer exists is dropped", async ({
        page,
    }) => {
        await page.goto("/", { waitUntil: "domcontentloaded" });
        await seedGuestCart(page, [
            { slug: "deleted-product-xyz", name: "Ghost", price: 5, qty: 1 },
        ]);
        await page.goto("/cart", { waitUntil: "networkidle" });
        ...
        await expect
            .poll(
                async () =>
                    (await page.locator("body").innerText()).includes("Ghost"),
                { timeout: 15_000 },
            )
            .toBe(false);
    });
