...
export default function ProfileSettings({ initial }) {
    ...
    const onPickFile = async (e) => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(json.error || t("settings.uploadFailed"));
            setImage(json.url);
        } catch (err) {
    ...
    const initials = (initial.name || initial.username || "?")
        .trim()
        .slice(0, 2)
        .toUpperCase();
        ...
        >
            <form onSubmit={onSubmit}>
                ...
                <div className="mt-6 grid sm:grid-cols-2 gap-4">
                    ...
                    >
                        <div className="flex rounded-xl border border-gray-200 bg-white focus-within:border-eco-green overflow-hidden">
                            <span className="inline-flex items-center px-3 bg-gray-50 text-gray-400 text-sm border-r border-gray-200">
                                @
                            </span>
                            <input
                                name="username"
                ...
                <div className="mt-5 flex justify-end">
                    <SubmitButton pending={pending || uploading}>
                        {t("settings.save")}
                    </SubmitButton>
                </div>
