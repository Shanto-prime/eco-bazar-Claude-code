...
export async function generateMetadata({ params }) {
    ...
    return {
        title: banner ? `${banner.title}   Ecobazar` : t("deals.notFoundTitle"),
    };
}
...
export default async function DealPage({ params }) {
    ...
    return (
        <>
            <Breadcrumb
                items={[
                    { href: "/shop", label: t("deals.breadcrumb") },
                    { label: banner.title },
                ]}
            />
            ...
            <section className="max-w-[1320px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3">
                    <h1 className="text-2xl sm:text-3xl font-bold mr-auto">
                        {banner.title}
                    </h1>
                    ...
                    deadline six hours off. */}
                                ...
                                {t("deals.endsLabel")}{" "}
                                <LocalTime value={banner.deadline} />
                            </span>
                        ))}
                </div>
