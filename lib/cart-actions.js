...
//
//   getCart()                 → { items, coupon } | null      (read)
//   saveCart(items, coupon)   → persist the current cart       (write)
//   mergeCart(items, coupon)  → merge a guest cart into the    (login)
//                               user's saved cart, return the merged result
//
...
export async function getCart() {
  ...
  return { items: safeItems(row.items), coupon: cleanCoupon(row.coupon) };
}

export async function saveCart(items, coupon = null) {
  const user = await getCurrentUser();
  ...
  await prisma.cart.upsert({
    ...
    update: { items: clean, coupon: cleanedCoupon },
    create: { userId: user.id, items: clean, coupon: cleanedCoupon },
  });
  return { ok: true };
}
...
export async function mergeCart(items, coupon = null) {
  ...
  await prisma.cart.upsert({
    ...
    create: { userId: user.id, items: merged, coupon: mergedCoupon },
  });

  return { items: merged, coupon: mergedCoupon };
}
