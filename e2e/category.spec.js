...
test.describe("Category search", () => {
    test("selecting a category shows only that category's products", async ({
        page,
    }) => {
    ...
    test("different categories show DIFFERENT products (bug fix)", async ({
        page,
    }) => {
        // Fresh Vegetables shows eggplant but not the apple…
        await page.goto("/shop?cat=fresh-vegetables", {
            waitUntil: "networkidle",
        });
        await expect(
            page.locator('a[href="/shop/eggplant"]').first(),
        ).toBeVisible();
        await expect(page.locator('a[href="/shop/green-apple"]')).toHaveCount(
            0,
        );
    ...
    test("the sidebar Categories list filters when clicked", async ({
        page,
    }) => {
