...
export async function requestReturnAction(input) {
  ...
  let data;
  try { data = Schema.parse(input); }
  catch { return { ok: false, error: "Invalid input." }; }

  const order = await prisma.order.findUnique({
    where:  { id: data.orderId },
    select: {
      id: true, number: true, status: true, userId: true,
      history: { orderBy: { createdAt: "asc" }, select: { status: true, createdAt: true } },
    },
  });

  const check = canRequestReturn({ viewerId: user.id, order, now: Date.now() });
  if (!check.ok) {
    const map = {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({
      data: {
        ...
        metadata: { number: order.number, from: "DELIVERED", to: "CANCELLED" },
      },
    });
  });
...
export async function submitReviewAction(input) {
  ...
  let data;
  try { data = ReviewSchema.parse(input); }
  catch { return { ok: false, error: "Invalid review (rating 1–5, body required)." }; }

  const order = await prisma.order.findUnique({
    where:  { id: data.orderId },
    select: {
      id: true, status: true, userId: true,
      items: { where: { productId: data.productId }, select: { id: true, productSlug: true } },
    },
  });
