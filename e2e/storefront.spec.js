...
test.describe("Storefront", () => {
    ...
    test("prices display in BDT (৳) by default", async ({ page }) => {
        ...
        await expect(
            page.getByText("৳", { exact: false }).first(),
        ).toBeVisible();
    });

    test("shop lists products and search filters by query", async ({
        page,
    }) => {
        await page.goto("/shop");
        await expect(page.locator('a[href^="/shop/"]').first()).toBeVisible();
    ...
    test("product detail page shows name, price and Add to Cart", async ({
        page,
    }) => {
        await page.goto("/shop/green-apple");
        await expect(
            page.getByRole("heading", { name: /green apple/i }),
        ).toBeVisible();
        await expect(
            page.getByText("৳", { exact: false }).first(),
        ).toBeVisible();
    ...
    test("unknown product slug shows a soft-404 with suggestions", async ({
        page,
    }) => {
