...
const OfferSchema = z.object({
  ...
  percentOff: z.coerce.number().int()
    .min(OFFER_MIN_PERCENT, `Discount must be at least ${OFFER_MIN_PERCENT}%.`)
    .max(OFFER_MAX_PERCENT, `Discount can't be more than ${OFFER_MAX_PERCENT}%.`),
...
function parseEndsAt(raw) {
  ...
  if (Number.isNaN(d.getTime())) throw new Error("That end date isn't valid.");
  if (d.getTime() <= Date.now())  throw new Error("The end date must be in the future.");
  return d;
}
...
async function assertNoOtherLiveOffer(productId, exceptId = null) {
  ...
  if (clash) throw new Error("That product already has a running offer. Edit or end that one first.");
}
...
export async function createOfferAction(formData) {
  ...
  catch (e) { return fail(e.issues?.[0]?.message || "Invalid offer."); }
  ...
  const created = await prisma.productOffer.create({
    data: { productId: product.id, percentOff: d.percentOff, endsAt, active: d.active, createdById: admin.id },
  });
  await prisma.auditLog.create({
    data: {
      actorId: admin.id, action: "offer.create", entity: "ProductOffer", entityId: created.id,
      metadata: { productSlug: product.slug, productName: product.name, percentOff: d.percentOff, endsAt: endsAt.toISOString() },
    },
  });
...
export async function updateOfferAction(offerId, formData) {
  ...
  await prisma.productOffer.update({
    ...
    data:  { productId: product.id, percentOff: d.percentOff, endsAt, active: d.active },
  });
  await prisma.auditLog.create({
    data: {
      actorId: admin.id, action: "offer.update", entity: "ProductOffer", entityId: existing.id,
      metadata: { productSlug: product.slug, percentOff: d.percentOff, endsAt: endsAt.toISOString() },
    },
  });
...
export async function deleteOfferAction(offerId) {
  ...
  await prisma.auditLog.create({
    data: {
      actorId: admin.id, action: "offer.delete", entity: "ProductOffer", entityId: existing.id,
      metadata: { productSlug: existing.product?.slug ?? null, percentOff: existing.percentOff },
    },
  });
...
export async function toggleOfferAction(offerId) {
  ...
  if (!existing.active) {
    try { await assertNoOtherLiveOffer(existing.productId, existing.id); }
    catch (e) { return fail(e.message); }
  }

  await prisma.productOffer.update({ where: { id: existing.id }, data: { active: !existing.active } });
  await prisma.auditLog.create({
    data: {
      actorId: admin.id, action: "offer.toggle", entity: "ProductOffer", entityId: existing.id,
      metadata: { active: !existing.active, productSlug: existing.product?.slug ?? null },
    },
  });
