...
export default function OrderDetails({ order }) {
  ...
  const [returnState, setReturnState] = useState({ error: null, notice: null });
  const [returnPending, startReturnTransition] = useTransition();

  const requestReturn = () => {
    if (!confirm(t("orders.returnConfirm", { number: order.number }))) return;
    setReturnState({ error: null, notice: null });
    startReturnTransition(async () => {
      const res = await requestReturnAction({ orderId: order.id });
      if (!res?.ok) {
        setReturnState({ error: res?.error || "Return request failed.", notice: null });
        return;
      }
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-2 text-sm">
                ...
                {order.viewerIsOwner && order.canReturn && (
                  ...
                  >
                    <i className="fa-solid fa-rotate-left" /> {t("orders.requestReturn")}
                  </button>
                )}
              </div>
              ...
              {order.viewerIsOwner && order.status === "DELIVERED" && order.returnDeadline && (
                <p className="text-xs text-gray-500">
                  {order.canReturn
                    ? t("orders.returnWindow", { days: RETURN_WINDOW_DAYS }) + " (" + fmtDate(order.returnDeadline) + ")"
                    : t("orders.returnWindowExpired")}
                </p>
              )}
              {returnState.error && <p className="text-xs text-red-600">{returnState.error}</p>}
              {returnState.notice && <p className="text-xs text-eco-green">{returnState.notice}</p>}

              {/* Ordered items — snapshot of what was purchased. */}
              {order.items && order.items.length > 0 && (
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 mb-2">{t("orders.itemsLabel")}</p>
                  <ul className="divide-y divide-gray-100 border border-gray-200 rounded-xl overflow-hidden">
                    {order.items.map((it) => (
                      <ItemRow
                        ...
                        item={it}
                        orderId={order.id}
                        canReview={order.canReviewItems && !it.reviewed && !!it.productId}
                  ...
                  <div className="mt-3 text-sm space-y-1">
                    <div className="flex justify-between text-gray-500">
                      <span>{t("orders.subtotal")}</span><span>{taka(order.subtotal)}</span>
                    </div>
                    {order.discount > 0 && (
                      <div className="flex justify-between text-gray-500">
                        <span>{t("orders.discount")}</span><span>−{taka(order.discount)}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-gray-500">
                      <span>{t("orders.shipping")}</span><span>{taka(order.shipping)}</span>
                    </div>
                    <div className="flex justify-between font-semibold pt-1 border-t border-gray-200 mt-1">
                      <span>{t("orders.total")}</span><span>{taka(order.total)}</span>
                    </div>
                  </div>
                </div>
              )}
...
function ItemRow({ item, orderId, canReview, alreadyReviewed }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    if (!body.trim()) { setError(t("orders.reviewPlaceholder")); return; }
    setError(null);
    startTransition(async () => {
      const res = await submitReviewAction({ orderId, productId: item.productId, rating, body });
    ...
    <li className="p-3 flex items-start justify-between gap-3">
      <div className="min-w-0 flex-1">
        ...
        {alreadyReviewed && !saved && (
          <p className="text-[11px] text-emerald-600 mt-1"><i className="fa-solid fa-check mr-1" />{t("orders.reviewYours", { stars: "" })}</p>
        )}
        {saved && (
          <p className="text-[11px] text-emerald-600 mt-1"><i className="fa-solid fa-check mr-1" />{t("orders.reviewSaved")}</p>
        )}
        {canReview && !expanded && !saved && (
          ...
          >
            <i className="fa-regular fa-star" /> {t("orders.writeReview")}
          </button>
        )}
        {expanded && (
      ...
      <div className="text-sm font-semibold whitespace-nowrap">{taka(item.lineTotal)}</div>
    </li>
  );
}
