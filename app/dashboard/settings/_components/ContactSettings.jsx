            ...
            {!editing ? (
                <div className="mt-1 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                        <p
                            className={`text-sm truncate ${value ? "text-gray-700" : "text-gray-400"}`}
                        >
    ...
    return (
        <div className="rounded-xl bg-gray-50 border border-gray-200 px-4 py-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm">
            <span className="text-gray-600">
                {t(req.field === "EMAIL" ? "settings.email" : "settings.phone")}
            </span>
            ...
            <span
                className={`inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full ${STATUS_STYLE[req.status]}`}
            >
...
export default function ContactSettings({
    ...
    const pendingFor = (field) =>
        requests.some((r) => r.field === field && r.status === "PENDING");
    ...
    const submit = (field, e) => {
        ...
        start(async () =>
            setResult(await requestContactChangeAction(formData)),
        );
    };

    const cancel = (id) =>
        start(async () => setResult(await cancelContactChangeAction(id)));
