...
export function CartProvider({ children, user = null }) {
    ...
    useEffect(() => {
        ...
        if (!userId) {
            if (prev) {
                dispatch({
                    type: "HYDRATE",
                    payload: { items: [], wishlist: [], coupon: null },
                });
                try {
                    localStorage.removeItem("ecobazar-cart-v1");
                } catch {}
    ...
    const removeItem = useCallback(
        (slug) => {
            ...
            if (i)
                showToast(t("toast.removedFromCart", { name: i.name }), "info");
        },
