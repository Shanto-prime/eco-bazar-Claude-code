...
export default function RegisterForm() {
  ...
  const onPickFile = (e) => {
    ...
    if (!["image/jpeg", "image/png", "image/webp"].includes(f.type)) {
      setError(t("settings.uploadFailed")); return;
    }
    if (f.size > 2 * 1024 * 1024) { setError(t("auth.photoTooLarge")); return; }
    setError(null);
    setFile(f);
  ...
  const onSubmit = async (e) => {
    ...
    if (file) {
      try {
        ...
        const up = await fetch("/api/upload/avatar", { method: "POST", body });
        const json = await up.json();
        if (up.ok && json.url) await updateAvatarAction(json.url);
      } catch { /* leave the avatar unset; user can add it in settings */ }
    }
  ...
  const initials = (form.name || form.username || "?").trim().slice(0, 2).toUpperCase();
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap gap-2">
                ...
                {preview && (
                  <button type="button" onClick={clearPhoto} className="rounded-md px-3.5 py-2 text-sm font-medium text-gray-500 hover:text-red-600 hover:bg-red-50 min-h-[40px]">
                    {t("settings.removePhoto")}
                  </button>
                )}
              </div>
              <p className="mt-1 text-xs text-gray-400">{t("auth.photoOptionalHint")}</p>
            </div>
          </div>

          <div>
            <label className="text-xs text-gray-500" htmlFor="name">{t("auth.fullName")}</label>
            <input id="name" required value={form.name} onChange={set("name")} autoComplete="name" className="eco-input" placeholder={t("auth.fullNamePh")} />
          </div>
          <div>
            <label className="text-xs text-gray-500" htmlFor="username">{t("auth.username")}</label>
            <input id="username" required value={form.username} onChange={set("username")} autoComplete="username" className="eco-input" placeholder={t("auth.usernamePh")} />
          </div>
          <div>
            <label className="text-xs text-gray-500" htmlFor="email">{t("checkout.email")}</label>
            <input id="email" type="email" required value={form.email} onChange={set("email")} autoComplete="email" className="eco-input" placeholder={t("auth.emailPh")} />
          </div>
          <div>
            <label className="text-xs text-gray-500" htmlFor="password">{t("auth.passwordHint")}</label>
            <input id="password" type="password" minLength={8} required value={form.password} onChange={set("password")} autoComplete="new-password" className="eco-input" placeholder="••••••••" />
          </div>
          <div>
            <label className="text-xs text-gray-500" htmlFor="phone">{t("auth.phoneOptional")}</label>
            <input id="phone" type="tel" value={form.phone} onChange={set("phone")} autoComplete="tel" className="eco-input" placeholder={t("auth.phonePh")} />
          </div>
          <button type="submit" disabled={busy} className="w-full py-3 rounded-md bg-eco-green text-white font-medium hover:bg-emerald-600 disabled:opacity-60 min-h-[44px]">
