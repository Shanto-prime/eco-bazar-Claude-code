  ...
  try {
    await prisma.$transaction(async (tx) => {
      if (req.type === "PRODUCT_DELETE") {
        const product = await tx.product.findUnique({ where: { id: req.entityId }, select: { slug: true, name: true } });
        if (product) {
          await tx.product.delete({ where: { id: req.entityId } });
          await tx.auditLog.create({
            data: { actorId: admin.id, action: "product.delete", entity: "Product", entityId: req.entityId,
                    metadata: { slug: product.slug, name: product.name, viaRequest: req.id, requesterId: req.requesterId } },
          });
        }
        ...
        if (order && !isTerminal(order.status)) {
          ...
          await tx.auditLog.create({
            data: { actorId: admin.id, action: "order.status.update", entity: "Order", entityId: order.id,
                    metadata: { number: order.number, from: order.status, to: "CANCELLED", viaRequest: req.id, requesterId: req.requesterId } },
          });
        }
      }
    ...
    return { ok: false, error: e?.message || "Could not approve the request." };
  }
...
export async function rejectRequest(input) {
  ...
  let data;
  try { data = RejectSchema.parse(input); }
  catch { return { ok: false, error: "Invalid input." }; }

  const req = await prisma.approvalRequest.findUnique({ where: { id: data.requestId }, select: { id: true, status: true, type: true, entityId: true } });
  if (!req) return { ok: false, error: "Request not found." };
  if (req.status !== "PENDING") return { ok: false, error: "This request was already reviewed." };
  ...
  await prisma.$transaction(async (tx) => {
    await tx.approvalRequest.update({
      where: { id: req.id },
      data:  { status: "REJECTED", reviewerId: admin.id, reviewedAt: new Date(), reviewNote: data.note || null },
    });
    await tx.auditLog.create({
      data: { actorId: admin.id, action: "approval.reject", entity: "ApprovalRequest", entityId: req.id, metadata: { type: req.type, entityId: req.entityId } },
    });
  });
