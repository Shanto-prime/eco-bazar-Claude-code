...
export default function DeleteUserButton({ userId, label, disabled = false, reason }) {
  const t = useT();
  ...
  if (disabled) {
    return <span className="text-xs text-gray-300" title={reason}>—</span>;
  }
  ...
  const onDelete = () => {
    ...
    startTransition(async () => {
      ...
      if (!res?.ok) { setError(res?.error || t("dashboard.userDeleteFailed")); return; }
      router.refresh();
    });
  };
  ...
  return (
    <div className="flex flex-col gap-1">
      <button
        type="button" onClick={onDelete} disabled={pending}
