  ...
  return (
    <section className="min-h-[70vh] grid place-items-center px-4 sm:px-6 py-10 sm:py-16 bg-eco-bg">
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border border-gray-200 p-5 sm:p-8">
        ...
        <h1 className="text-xl sm:text-2xl font-bold mb-1">Forgot your password?</h1>
        <p className="text-sm text-gray-500 mb-5">
          Enter your username or email and we&apos;ll send a reset link to the email on file.
        </p>

        {sent ? (
          <div className="rounded-md bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm px-4 py-3">
            If an account matches <b>{identifier}</b>, a reset link is on its way to the email on file.
            Check your inbox — in dev, the link is printed to the server console.
          </div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-3">
            <div>
              <label className="text-xs text-gray-500" htmlFor="identifier">Username or email</label>
              <input
                id="identifier" required autoComplete="username"
                value={identifier} onChange={(e) => setIdentifier(e.target.value)}
                className="eco-input" placeholder="admin or you@example.com"
              />
            </div>
            <button
