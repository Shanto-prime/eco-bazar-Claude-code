...
export async function lookupOrderAction(formData) {
    ...
    const rl = rateLimit(`order-lookup:${ip}`, {
        limit: 20,
        windowMs: 15 * 60 * 1000,
    });
    if (!rl.ok)
    ...
    try {
        ...
        return {
            ok: false,
            error: "Enter the order number and the email you used.",
        };
    }
    ...
    if (!order)
        return { ok: false, error: "No order matches that number and email." };
    ...
    return {
        ...
        order: {
            ...
            items: order.items.map((i) => ({
                ...i,
                unitPrice: toDollars(i.unitPrice),
            })),
