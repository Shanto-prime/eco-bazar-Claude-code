...
test.afterAll(async () => {
  ...
  if (cartBefore) {
    await prisma.cart.update({
      where: { userId },
      data:  { items: cartBefore.items, coupon: cartBefore.coupon, wishlist: cartBefore.wishlist },
    });
  } else {
...
test.describe("Fix 4 — wishlist shows every saved product", () => {
  ...
  test("the oldest product in the catalogue still appears", async ({ page }) => {
    ...
    await expect(page.locator(`a[href="/shop/${oldest.slug}"]`).first()).toBeVisible();
  });

  test("only the saved products are rendered, not the catalogue", async ({ page }) => {
    const [a, b] = await prisma.product.findMany({ take: 2, select: { slug: true } });
    await setWishlist([a.slug, b.slug]);
    await page.goto("/wishlist", { waitUntil: "networkidle" });

    // Exactly the two saved products — the page used to receive up to 200.
    await expect(page.locator(`a[href="/shop/${a.slug}"]`).first()).toBeVisible();
    await expect(page.locator(`a[href="/shop/${b.slug}"]`).first()).toBeVisible();
    await expect.poll(() => page.locator('a[href^="/shop/"]').count(), { timeout: 10_000 })
  ...
  test("a wishlisted product that has since been deleted is skipped, not crashed on", async ({ page }) => {
