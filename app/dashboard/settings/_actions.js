...
const ProfileSchema = z.object({
    ...
    username: z
        ...
        .regex(
            /^[a-z0-9._-]+$/,
            "Use lowercase letters, digits, dot, underscore or dash.",
        )
        .optional()
    ...
    image: z
        .string()
        .max(500)
        .refine(isSafeImageUrl, "That image link isn't valid.")
        .optional()
        .or(z.literal("").transform(() => undefined)),
});
...
export async function updateAvatarAction(imageUrl) {
    ...
    if (raw && !isSafeImageUrl(raw))
        return fail("That image link isn't valid.");
...
export async function requestContactChangeAction(formData) {
    ...
    const value =
        data.field === "EMAIL" ? data.value.toLowerCase() : data.value;
    ...
    if (pending) {
        return fail(
            "You already have a pending request for this field. Cancel it first.",
        );
    }
...
export async function changePasswordAction(formData) {
    ...
    if (!me?.passwordHash) {
        return fail(
            "This account signs in with Google/Facebook and has no password.",
        );
    }
    ...
    await prisma.user.update({
        where: { id: session.id },
        data: {
            passwordHash: await bcrypt.hash(data.newPassword, BCRYPT_COST),
        },
    });
    ...
    .superRefine((d, ctx) => {
        ...
        if (d.city && !isValidDistrict(d.state, d.city)) {
            ctx.addIssue({
                code: "custom",
                path: ["city"],
                message: "Choose a district inside the selected division.",
            });
            return;
        }
        if (d.thana && !isValidThana(d.state, d.city, d.thana)) {
            ctx.addIssue({
                code: "custom",
                path: ["thana"],
                message: "Choose a thana inside the selected district.",
            });
        }
    });
...
export async function updateStoreCurrencyAction(formData) {
    ...
    for (const code of CURRENCY_CODES) {
        ...
        if (!Number.isFinite(raw) || raw <= 0) {
            return fail(
                `Enter a valid exchange rate for ${code} (BDT per 1 ${code}).`,
            );
        }
        rates[code] = raw;
    }
