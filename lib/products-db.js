...
export async function listProductsBySlugs(slugs = []) {
    const wanted = [
        ...new Set(slugs.filter((s) => typeof s === "string" && s)),
    ];
...
export async function listBestSellers(n = 10) {
    ...
    if (out.length < n) {
        const offered = await prisma.product.findMany({
            where: {
                OR: [{ badge: { not: null } }, { oldPrice: { not: null } }],
            },
            orderBy: { createdAt: "desc" },
            take: n * 2,
            include: PRODUCT_INCLUDE,
...
//   minPrice    base-currency MAJOR units (Taka), inclusive; undefined/0 = no floor
//   maxPrice    base-currency MAJOR units (Taka), inclusive; undefined = no cap
...
export async function queryProducts({
    ...
    // Price stored in base minor units (poisha). Only apply a bound when it's
    // explicitly given   undefined on either side means no cap.
    const price = {};
    if (minPrice != null && Number(minPrice) > 0)
        price.gte = toCents(Number(minPrice));
    if (maxPrice != null && Number.isFinite(Number(maxPrice)))
        price.lte = toCents(Number(maxPrice));
...
// Min and max product price in the catalogue, in base-currency MAJOR units
// (Taka). Drives the shop's price-range slider bounds so the two knob labels
// always sit at the cheapest and most expensive product currently on sale
// rather than a hard-coded 0..100. Rounds outward so no product ends up
// outside the slider range because of a fractional poisha price.
//
// Returns { min: 0, max: 0 } when the catalogue is empty; the caller (see
// app/shop/page.js) is expected to substitute a sensible fallback.
export async function getPriceBounds() {
    const agg = await prisma.product.aggregate({
        _min: { price: true },
        _max: { price: true },
    });
    const minMinor = agg._min.price ?? 0;
    const maxMinor = agg._max.price ?? 0;
    return {
        min: Math.floor(minMinor / 100),
        max: Math.ceil(maxMinor / 100),
    };
}
