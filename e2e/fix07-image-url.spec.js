...
async function submitProfileWithImage(page, value) {
    ...
    await page
        .getByRole("button", {
            name: /save profile|update profile|save changes/i,
        })
        .first()
        .click();
...
test.describe("Fix 7   profile image URLs are validated", () => {
    ...
    test("an https URL is accepted (OAuth avatars must keep working)", async ({
        page,
    }) => {
    ...
    test("a data: URL is rejected", async ({ page }) => {
        const text = await submitProfileWithImage(
            page,
