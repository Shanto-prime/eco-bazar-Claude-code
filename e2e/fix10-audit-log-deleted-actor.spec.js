...
test.describe("Fix 10 — audit log survives a deleted actor", () => {
  ...
  test("an audit row whose actor was deleted renders instead of crashing", async ({ page }) => {
    ...
    const res = await page.goto("/dashboard/audit-log", { waitUntil: "networkidle" });
  ...
  test("rows with a live actor still show who did it", async ({ page }) => {
    ...
    await prisma.auditLog.create({
      data: { actorId: admin.id, action: ORPHAN_ACTION, entity: "User", entityId: admin.id },
    });
