...
async function placeGuestOrder(page) {
  ...
  await seedGuestCart(page, [{ slug: SLUG, name: "Green Apple", price: 14.99, qty: 1 }]);
  ...
  expect(num, "the confirmation screen must show an order number").toBeTruthy();
  return num;
}
...
test.describe("Fix 6 — a guest can find their order", () => {
  test("the confirmation screen gives a guest a way to use the number", async ({ page }) => {
  ...
  test("the right number with the WRONG email is refused", async ({ page }) => {
    const num = await placeGuestOrder(page);
  ...
  test("a made-up number is refused with the same generic message", async ({ page }) => {
  ...
  test("the lookup page is reachable without signing in", async ({ page }) => {
    const res = await page.goto("/orders/lookup", { waitUntil: "domcontentloaded" });
