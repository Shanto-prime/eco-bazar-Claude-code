...
const BannerSchema = z.object({
    ...
    targetTag: z
        .string()
        .trim()
        .min(1, "Enter the badge/tag applicable products carry.")
        .max(60),
...
async function assertSlugUnique(slug, exceptId = null) {
    ...
    if (clash)
        throw new Error(`Slug "${slug}" is already used by another banner.`);
}
...
export async function createBannerAction(formData) {
    ...
    try {
        ...
        return fail(e.issues?.[0]?.message || "Invalid banner.");
    }

    try {
        await assertSlugUnique(d.slug);
    } catch (e) {
    ...
    await prisma.auditLog.create({
        data: {
            ...
            entity: "PromoBanner",
            entityId: created.id,
            metadata: { slug: d.slug, placement: d.placement },
        },
    });
...
export async function updateBannerAction(bannerId, formData) {
    ...
    try {
        await assertSlugUnique(d.slug, existing.id);
    } catch (e) {
        return fail(e.message);
    }
