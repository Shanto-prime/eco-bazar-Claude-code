...
function readAdminConfig() {
  ...
  if (password.length < 12) {
    console.warn(`⚠  SEED_ADMIN_PASSWORD is only ${password.length} characters. ` +
...
async function seedAdmin(cfg) {
  ...
  console.log(`• Admin ${claimSuper ? "(super)" : ""} ready: ${admin.email} (${admin.username})`);
  if (existingSuper && existingSuper.email !== cfg.email) {
    console.log(`  Note: super-admin flag stays with ${existingSuper.email}.`);
  }
  return admin;
}
...
async function main() {
  ...
  console.log("");
  console.log("Done. Sign in at /login with the SEED_ADMIN_EMAIL + SEED_ADMIN_PASSWORD.");
