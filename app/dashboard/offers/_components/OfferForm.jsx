...
import { OFFER_MIN_PERCENT, OFFER_MAX_PERCENT, clampPercent } from "../../../../lib/offers";
...
export default function OfferForm({ offer, header = null, onDone, onCancel }) {
  ...
  const now  = base != null ? Math.max(0.01, Math.round(base * (100 - clampPercent(percent))) / 100) : null;
    ...
    <form onSubmit={onSubmit} className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <label className="block">
          <span className="block text-[13px] font-medium mb-1.5">
            {t("offers.percentOff")} <span className="text-eco-green">*</span>
          </span>
          <div className="flex rounded-xl border border-gray-200 bg-white focus-within:border-eco-green overflow-hidden">
            <input
              ...
              min={OFFER_MIN_PERCENT} max={OFFER_MAX_PERCENT} step="1" required
              value={percent}
              onChange={(e) => setPercent(e.target.value)}
              className="w-full px-3.5 py-2.5 text-sm bg-transparent focus:outline-none"
            />
            <span className="inline-flex items-center px-3 bg-gray-50 text-gray-400 text-sm border-l border-gray-200">%</span>
          </div>
          <span className="block mt-1 text-xs text-gray-400">
            {t("offers.percentHint", { min: OFFER_MIN_PERCENT, max: OFFER_MAX_PERCENT })}
          </span>
        </label>

        <label className="block">
          <span className="block text-[13px] font-medium mb-1.5">
            {t("offers.endsAt")} <span className="text-eco-green">*</span>
          </span>
          <input
            name="endsAt" type="datetime-local" required
            defaultValue={offer?.endsAt ? toLocalInput(offer.endsAt) : tomorrowLocalInput()}
            className="eco-input rounded-xl"
          />
          <span className="block mt-1 text-xs text-gray-400">{t("offers.endsAtHint")}</span>
        </label>
      </div>
      ...
      {base != null && (
        <p className="mt-4 rounded-xl bg-emerald-50 border border-emerald-100 px-3.5 py-2.5 text-sm text-emerald-800">
          ...
          {t("offers.pricePreview", { now: money(now), was: money(base) })}
        </p>
      )}

      <label className="flex items-center gap-2 text-sm mt-4 cursor-pointer">
        <input type="checkbox" name="active" className="eco-check" defaultChecked={offer ? offer.active : true} />
        {t("offers.active")}
      </label>
