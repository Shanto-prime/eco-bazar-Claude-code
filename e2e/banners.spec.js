...
test.describe("Promo banners   storefront + landing", () => {
    test("a banner on the homepage links to its /deals page", async ({
        page,
    }) => {
    ...
    test("the /deals landing shows only the applicable products", async ({
        page,
    }) => {
        ...
        await expect(
            page.locator('a[href="/shop/green-capsicum"]').first(),
        ).toBeVisible();
    ...
    test("an unknown deal slug 404s", async ({ page }) => {
        const res = await page.goto("/deals/no-such-deal-xyz", {
...
test.describe("Promo banners   admin", () => {
    test("admin sees the Banners manager with the seeded banners", async ({
        page,
    }) => {
        await login(page, "admin", "admin");
        await page.goto("/dashboard/banners", { waitUntil: "networkidle" });
        await expect(
            page.getByRole("heading", { name: /promo banners/i }),
        ).toBeVisible();
        await expect(
            page.getByRole("link", { name: "/deals/top-deals" }),
        ).toBeVisible();
        await expect(
