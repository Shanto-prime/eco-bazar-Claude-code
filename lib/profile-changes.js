...
// Only ADMIN skips the review queue for their own email/phone change. A
// MODERATOR's change goes through the queue like a customer's   the admin
// approves requests from BOTH users and moderators. (Moderators no longer
// self-approve: the admin dashboard is the single place those decisions are
// made.) Single source of truth for that rule.
export const SELF_APPROVE_ROLES = ["ADMIN"];
...
export async function notifyContactChange({ field, newValue, previousEmail }) {
    try {
        if (field === "EMAIL") {
            ...
            if (
                previousEmail &&
                previousEmail.toLowerCase() !== newValue.toLowerCase()
            ) {
                await sendMail({
                    to: previousEmail,
