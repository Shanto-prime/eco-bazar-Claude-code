...
const AREAS = [
    {
        ...
        kind: "banner",
        placement: "TOP",
        labelKey: "banners.areaTop",
    },
    {
        value: "BELOW_LIST",
        kind: "banner",
        placement: "BELOW_LIST",
        labelKey: "banners.areaBelowList",
    },
    {
        value: "HOT_DEALS_OFFER",
        kind: "offer",
        labelKey: "banners.areaHotDealsOffer",
    },
];
...
export default function BannerForm({ banner, offer, onDone, onCancel }) {
    ...
    const [area, setArea] = useState(
        editingOffer
            ? "HOT_DEALS_OFFER"
            : banner?.placement && banner.placement !== "HOT_DEALS"
              ? banner.placement
              : "TOP",
    );
    ...
    const onPickFile = async (e) => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(json.error || t("banners.uploadFailed"));
            setImage(json.url);
        } catch (err) {
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <label className="block">
                    ...
                    <input
                        name="targetTag"
                        defaultValue={banner?.targetTag || ""}
                        required
                        maxLength={60}
                        className="eco-input rounded-xl"
                        placeholder={t("banners.targetTagPh")}
                    />
                    <span className="block mt-1 text-xs text-gray-400">
                        {t("banners.targetTagHint")}
                    </span>
                </label>
                ...
                <label className="block">
                    ...
                    <input
                        name="promoCode"
                        defaultValue={banner?.promoCode || ""}
                        maxLength={40}
                        className="eco-input rounded-xl"
                        placeholder={t("banners.promoCodePh")}
                    />
                </label>
                ...
                <label className="block">
                    ...
                    <input
                        name="deadline"
                        type="datetime-local"
                        defaultValue={toLocalInput(banner?.deadline)}
                        className="eco-input rounded-xl"
                    />
                    <span className="block mt-1 text-xs text-gray-400">
                        {t("banners.deadlineHint")}
                    </span>
                </label>
