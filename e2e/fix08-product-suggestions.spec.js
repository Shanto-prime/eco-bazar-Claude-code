...
test.describe("Fix 8   soft-404 suggestions come from the database", () => {
    test("a near-miss slug still suggests the right product", async ({
        page,
    }) => {
        ...
        await expect(
            page.locator('a[href="/shop/green-apple"]').first(),
        ).toBeVisible();
    });

    test("every suggested link resolves to a real product page", async ({
        page,
    }) => {
        await page.goto("/shop/totally-unknown-slug-xyz", {
        ...
        expect(
            hrefs.length,
            "the 404 page should offer something",
        ).toBeGreaterThan(0);
    ...
    test("suggested cards show the current DB price", async ({ page }) => {
        const product = await prisma.product.findUnique({
            ...
            select: {
                ...
                offers: {
                    where: { active: true },
                    select: { percentOff: true, endsAt: true },
                },
            },
        });
        ...
        const live = product.offers.find(
            (o) => new Date(o.endsAt).getTime() > Date.now(),
        );
        const minor = live
            ? Math.max(
                  1,
                  Math.round((product.price * (100 - live.percentOff)) / 100),
              )
            : product.price;
    ...
    test("a product deleted from the DB is never suggested", async ({
        page,
    }) => {
        ...
        const created = await prisma.product.create({
            data: {
                slug: "e2e-fix08-ghost-berry",
                name: "E2E Fix08 Ghost Berry",
                price: 1234,
                stock: 5,
                createdById: (
        ...
        await page.goto("/shop/e2e-fix08-ghost-bery", {
            waitUntil: "networkidle",
        });

        await expect(
            page.locator('a[href="/shop/e2e-fix08-ghost-berry"]'),
        ).toHaveCount(0);
    });
});
