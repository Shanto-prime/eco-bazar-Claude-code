...
test.describe("Fix 3   timestamps render in the viewer's timezone", () => {
    ...
    for (const { path } of PAGES) {
        test(`${path} renders <time datetime> rather than a server-formatted string`, async ({
            page,
        }) => {
            ...
            test.skip(
                count === 0,
                `${path} has no timestamps to check on this dataset`,
            );
    ...
    test("the same order timestamp reads differently in Dhaka and UTC", async ({
        browser,
    }) => {
        ...
        const read = async (timezoneId) => {
            ...
            const text = norm(
                await page.locator("time[datetime]").first().innerText(),
            );
            await ctx.close();
            return text;
        };
    ...
    test("a promo deadline on /deals is client-formatted", async ({ page }) => {
        ...
        test.skip(
            !banner,
            "no active banner with a deadline   set one to exercise this",
        );
