...
function isDuplicateNumber(err) {
  return err?.code === "P2002" && String(err?.meta?.target ?? "").includes("number");
}
...
export async function placeOrderAction({ billing, items, couponCode }) {
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      result = await runPlaceOrder({ billingData, lines, couponCode, userId });
      break;
    } catch (err) {
  ...
  return { ok: true, orderId: result.id, number: result.number, total: toDollars(result.total), signedIn: Boolean(userId) };
}
...
async function runPlaceOrder({ billingData, lines, couponCode, userId }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const orderItemsData = lines.map((l) => {
      ...
      const unit = offer ? discountedMinor(p.price, offer.percentOff) : p.price; // integer cents
      subtotal += unit * l.qty;
    ...
    if (couponCode) {
      ...
      if (c) discount = c.type === "percent" ? Math.round(subtotal * (c.value / 100)) : c.value * 100;
