...
import { promoteIfFirstUser, claimGuestOrdersForUser } from "./user-service";
...
export const { handlers, auth, signIn, signOut } = NextAuth({
    ...
    providers: [
        Credentials({
            ...
            async authorize(creds, request) {
                ...
                const byId = rateLimit(`login:id:${identifier}`, {
                    limit: 10,
                    windowMs: 15 * 60 * 1000,
                });
    ...
    events: {
        ...
        //
        // Also claim any past guest orders placed with this email   an OAuth
        // sign-up follows the same "your prior orders show up in your dashboard"
        // rule as the credentials route.
        async createUser({ user }) {
            await promoteIfFirstUser(user.id);
            if (user.email) await claimGuestOrdersForUser(user.id, user.email);
        },
