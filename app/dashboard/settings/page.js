...
export default async function DashboardSettings() {
    ...
    const lastChangedFor = (field) => {
        const r = requests.find(
            (x) => x.field === field && x.status === "APPROVED",
        );
        if (!r) return null;
        return {
            date: (r.reviewedAt || r.createdAt).toISOString(),
            added: !r.currentValue,
        };
    };
    const lastChanged = {
        EMAIL: lastChangedFor("EMAIL"),
        PHONE: lastChangedFor("PHONE"),
    };
    ...
    const idleHours =
        user.role === "ADMIN" || user.role === "MODERATOR" ? 12 : 6;
