      ...
      {value ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-gray-500">{money(value.basePrice ?? value.price)}</div>
          </div>
          <button
            type="button"
        ...
        <div ref={boxRef} className="relative">
          <div className="flex rounded-xl border border-gray-200 bg-white focus-within:border-eco-green overflow-hidden">
            ...
            />
            {busy && <span className="inline-flex items-center pr-3 text-gray-400"><i className="fa-solid fa-spinner fa-spin text-xs" /></span>}
          </div>
          ...
          {open && (
            <ul className="absolute z-30 left-0 right-0 mt-1 max-h-72 overflow-auto rounded-xl border border-gray-200 bg-white shadow-lg">
              {visible.length === 0 && !busy && (
                <li className="px-3 py-2.5 text-sm text-gray-400">{t("offers.noResults")}</li>
              )}
              {visible.map((p) => (
                <li key={p.id}>
                  <button
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block text-xs text-gray-400">{money(p.basePrice ?? p.price)}</span>
                    </span>
                  </button>
                </li>
              ))}
          ...
          <span className="block mt-1 text-xs text-gray-400">
            {query.length > 0 && !searchable ? t("offers.searchHint") : t("offers.productHint")}
          </span>
        </div>
      )}
    </div>
