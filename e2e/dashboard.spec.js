...
test.describe("Dashboard   admin", () => {
    ...
    test("admin sees admin-only nav (Users, Profile requests, Audit log)", async ({
        page,
    }) => {
        ...
        await expect(
            nav.getByRole("link", { name: /profile requests/i }),
        ).toBeVisible();
        await expect(
            nav.getByRole("link", { name: /audit log/i }),
        ).toBeVisible();
    });

    test("settings page shows Profile, Email & phone, Password, Addresses", async ({
        page,
    }) => {
        ...
        await expect(
            page.getByRole("heading", { name: /email .* phone/i }),
        ).toBeVisible();
        await expect(
    ...
    test("admin-only Store currency card is present on settings", async ({
        page,
    }) => {
        await page.goto("/dashboard/settings");
        await expect(
            page.getByRole("heading", { name: /store currency/i }),
        ).toBeVisible();
    ...
    test("profile-requests queue page loads", async ({ page }) => {
        ...
        await expect(
            page.getByRole("heading", { name: /profile requests/i }),
        ).toBeVisible();
    });
});
...
test.describe("Dashboard   customer", () => {
    ...
    test("customer does NOT see admin-only sections", async ({ page }) => {
        ...
        await expect(nav.getByRole("link", { name: /audit log/i })).toHaveCount(
            0,
        );
    });
    ...
    test("customer is redirected away from the admin-only users page", async ({
        page,
    }) => {
