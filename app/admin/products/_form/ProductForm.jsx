...
// app/admin/products/_form/ProductForm.jsx
...
export default function ProductForm({ product, action, allowDelete = false, onDelete }) {
  ...
  return (
    <form onSubmit={onSubmit} className="bg-white border border-gray-200 rounded-lg p-5 sm:p-6 max-w-3xl">
      {error && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-xs text-gray-500">Slug * (URL: /product/<i>slug</i>)</label>
      ...
      <div className="mt-6">
        ...
        <label className="inline-flex items-center gap-2 px-4 py-2 border border-dashed border-gray-300 rounded-md cursor-pointer hover:border-eco-green text-sm">
      ...
      <div className="mt-8 flex flex-wrap gap-3 justify-between">
        <div className="flex gap-3">
          <button
            ...
            className="px-6 py-3 rounded-md bg-eco-green text-white font-medium hover:bg-emerald-600 disabled:opacity-60"
          >
            {pending ? "Saving…" : product ? "Save changes" : "Create product"}
          </button>
          <button type="button" onClick={() => router.push("/admin/products")} className="px-6 py-3 rounded-md border border-gray-200">
            Cancel
          </button>
        </div>
        {allowDelete && (
          <button
            ...
            className="px-6 py-3 rounded-md border border-red-300 text-red-600 hover:bg-red-50"
          >
